...
module.exports = React.createClass({
  ...
  validate: function() {
    ...
    if (validator.isEmail(this.state.username)) {
      ...
      !validator.isLength(this.state.username, 4, 30)
    ) {
  ...
  handleBlur: function() {
    this.setState({
      errors: _.omit(this.validate(), (val, key) => !this.state[key].length)
    });
  },
  ...
  render: function() {
    ...
    return (
      <form className="form-connect">
        ...
        <a className="link" onClick={this.handleClickForgotPassword}>
        ...
        <br />
        <div className="extra" style={{ color: "white" }}>
          Don't have an account yet?{" "}
          <a disabled={this.state.loading} onClick={this.handleClickSignup}>
            Sign Up
          </a>
        </div>
      </form>
