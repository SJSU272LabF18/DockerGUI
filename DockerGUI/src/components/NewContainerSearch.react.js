...
module.exports = React.createClass({
  ...
  search: function(query, page = 1) {
    ...
    if (this.state.query === query) {
      ...
      nextPage =
        page + 1 > this.state.totalPage ? this.state.totalPage : page + 1;
    ...
    _searchPromise = Promise.delay(200)
      .cancellable()
      .then(() => {
  ...
  render: function() {
    ...
    let repos = _.values(this.state.repos)
      .filter(repo => {
        if (repo.is_recommended || repo.is_user_repo) {
          return (
            repo.name.toLowerCase().indexOf(this.state.query.toLowerCase()) !==
              -1 ||
            repo.namespace
              .toLowerCase()
    ...
    if (this.state.previousPage) {
      ...
      for (
        ...
        previous.push(
          <li>
            <a href="" onClick={this.handlePage.bind(this, previousPage)}>
              {previousPage}
            </a>
          </li>
        );
    ...
    if (this.state.nextPage) {
      ...
      for (nextPage; nextPage < this.state.totalPage; nextPage++) {
        next.push(
          <li>
            <a href="" onClick={this.handlePage.bind(this, nextPage)}>
              {nextPage}
            </a>
          </li>
        );
    ...
    let current = (
      <li className="active">
        <span>
          {this.state.currentPage} <span className="sr-only">(current)</span>
        </span>
      </li>
    );
    paginateResults =
      (next.length || previous.length) && this.state.query !== "" ? (
        <nav>
    ...
    if (
      ...
      this.state.error.message.indexOf("getaddrinfo ENOTFOUND") !== -1
    ) {
      errorMsg = "There was an error contacting Docker Hub.";
    } else {
      errorMsg = this.state.error.message.replace(
        "HTTP code is 409 which indicates error: conflict - ",
        ""
      );
    }
    if (this.state.error) {
      ...
      results = (
        <div className="no-results">
          <h2>
            ...
            <Router.Link to="signup">Sign Up</Router.Link> to access your Docker
            Hub repositories.
          </h2>
      ...
      let spinner = this.state.accountLoading ? (
        <div className="spinner la-ball-clip-rotate la-dark">
          <div />
        </div>
      ) : null;
      results = (
      ...
      let validImages = this.state.images.filter(
      ...
      let userImageResults = userImageItems.length ? (
        <div className="result-grids">
          <div>
            <h4>My Images</h4>
            <div className="result-grid">{userImageItems}</div>
          </div>
        </div>
      ) : (
        <div className="no-results">
      ...
      results = { userImageResults };
    } else if (this.state.loading) {
      results = (
        <div className="no-results">
          <div className="loader">
            <h2>Loading Images</h2>
            <div className="spinner la-ball-clip-rotate la-dark la-lg">
              <div />
            </div>
          </div>
        </div>
      ...
      let recommendedItems = repos
        .filter(repo => repo.is_recommended)
        .map((image, index) => {
          const key = `rec-${image.name}-${index}`;
          return <ImageCard key={key} image={image} />;
        });
      let otherItems = repos
        .filter(repo => !repo.is_recommended && !repo.is_user_repo)
        .map((image, index) => {
      ...
      let recommendedResults = recommendedItems.length ? (
        <div>
          <h4>Other Widely Known Images</h4>
          <div className="result-grid">{recommendedItems}</div>
        </div>
      ) : null;

      let userRepoItems = repos
        .filter(repo => repo.is_user_repo)
        .map((image, index) => {
      ...
      let userRepoResults = userRepoItems.length ? (
        <div>
          ...
          <div className="result-grid">{userRepoItems}</div>
        </div>
      ) : null;
      ...
      if (otherItems.length) {
        otherResults = (
          <div>
            ...
            <div className="result-grid">{otherItems}</div>
          </div>
        );
      } else {
        otherResults = null;
        ...
        <div className="new-container">
          <div className="new-container-header">
            ...
            <div className="results-filters">
              <span
                className={`results-filter results-all tab ${
                  filter === "all" ? "active" : ""
                }`}
          ...
          <div className="pagination-center">{paginateResults}</div>
        </div>
      </div>
    );
  }
